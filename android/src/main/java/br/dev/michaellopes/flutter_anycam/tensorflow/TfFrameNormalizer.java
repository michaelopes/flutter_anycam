...
/*
    ...
    public ByteBufferPoolUtil.PoolItem normalize(
      ...
      /*  int width = argbBuffer.getWidth();
        ...
        ByteBuffer src = argbBuffer.asBuffer();
        src.rewind();
        IntBuffer srcInt = src.asIntBuffer();



        // Calcula scale/offset igual ao código atual...
        float scaleR, scaleG, scaleB, offsetR, offsetG, offsetB;
        switch (normalization) {
            case "simple":
                scaleR = scaleG = scaleB = 1.0f / 255.0f;
        ...
        // Pré-calcula LUT para os 256 valores possíveis de cada canal
        float[] lutR = new float[256];
        float[] lutG = new float[256];
        float[] lutB = new float[256];
        for (int v = 0; v < 256; v++) {
            lutR[v] = v * scaleR + offsetR;
            lutG[v] = v * scaleG + offsetG;
            lutB[v] = v * scaleB + offsetB;
        }
        ...
        switch (dataType) {
            case "float32": {
                ...
                for (int i = 0; i < pixelCount; i++) {
                    int pixel = srcInt.get(i);
                    out.put(lutR[(pixel >> 16) & 0xFF]);
                    out.put(lutG[(pixel >>  8) & 0xFF]);
                    out.put(lutB[ pixel        & 0xFF]);
                }
                break;
            }
            case "uint8": {
                // LUT direto como byte para uint8
                byte[] lutRb = new byte[256];
                byte[] lutGb = new byte[256];
                byte[] lutBb = new byte[256];
                for (int v = 0; v < 256; v++) {
                    lutRb[v] = (byte) Math.max(0, Math.min(255, Math.round(lutR[v])));
                    lutGb[v] = (byte) Math.max(0, Math.min(255, Math.round(lutG[v])));
                    lutBb[v] = (byte) Math.max(0, Math.min(255, Math.round(lutB[v])));
                }
                for (int i = 0; i < pixelCount; i++) {
                    int pixel = srcInt.get(i);
                    output.put(lutRb[(pixel >> 16) & 0xFF]);
                    output.put(lutGb[(pixel >>  8) & 0xFF]);
                    output.put(lutBb[ pixel        & 0xFF]);
                }
                break;
            }
            case "int8": {
                ...
                byte[] lutBb = new byte[256];
                for (int v = 0; v < 256; v++) {
                    int qR = Math.round(lutR[v] * invScale) + zeroPoint;
                    int qG = Math.round(lutG[v] * invScale) + zeroPoint;
                    int qB = Math.round(lutB[v] * invScale) + zeroPoint;
                    lutRb[v] = (byte) Math.max(-128, Math.min(127, qR));
                    lutGb[v] = (byte) Math.max(-128, Math.min(127, qG));
                    lutBb[v] = (byte) Math.max(-128, Math.min(127, qB));
                }
                for (int i = 0; i < pixelCount; i++) {
                    int pixel = srcInt.get(i);
                    output.put(lutRb[(pixel >> 16) & 0xFF]);
